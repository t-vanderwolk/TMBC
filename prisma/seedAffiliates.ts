...
type CanonicalAffiliate = {
  ...
  blogCta?: "Shop" | "Explore" | "Learn More";
};

const CANONICAL_AFFILIATES: CanonicalAffiliate[] = [
  {
...
function persistMetadata(metadata: Metadata) {
  fs.writeFileSync(metadataPath, JSON.stringify(metadata, null, 2) + "\n", "utf-8");
}

function buildVisibility(entry: CanonicalAffiliate, existing?: Record<string, unknown>) {
  const prior = existing?.visibility as
    | { blogEligible?: boolean; registryEligible?: boolean; mentorVisible?: boolean }
    | undefined;
  return {
...
function buildBlogSettings(entry: CanonicalAffiliate, existing?: Record<string, unknown>) {
  const prior = existing?.blogSettings as Record<string, unknown> | undefined;
  return {
    eligible: entry.blogEligible ?? prior?.eligible ?? entry.role !== "INFRASTRUCTURE",
    defaultCta: (prior?.defaultCta as string) ?? entry.blogCta ?? "Shop",
    placement: "END_CARD",
    primaryEligible: prior?.primaryEligible ?? true,
  };
}

function buildRegistrySettings(entry: CanonicalAffiliate, existing?: Record<string, unknown>) {
  const prior = existing?.registrySettings as Record<string, unknown> | undefined;
  return {
    retailerTier: entry.retailerTier ?? (prior?.retailerTier as string | undefined) ?? undefined,
    priority:
      entry.priority ?? (typeof prior?.priority === "number" ? prior.priority : undefined) ?? undefined,
    categoryExclusions: entry.categoryExclusions ?? (prior?.categoryExclusions as string[]) ?? [],
    fallbackToBrandDirect:
      entry.fallbackToBrandDirect ?? (prior?.fallbackToBrandDirect as boolean) ?? false,
  };
}
...
async function seed() {
  ...
  for (const affiliate of CANONICAL_AFFILIATES) {
    const targetCreate: Prisma.AffiliatePartnerUncheckedCreateInput = {
      id: randomUUID(),
      name: affiliate.name,
    ...
    metadata.partners[affiliate.name] = {
      ...existingMeta,
      category: affiliate.categories?.[0] ?? (existingMeta.category as string | undefined) ?? null,
      role: affiliate.role,
      ...
      affiliateIds: {
        ...(existingMeta.affiliateIds as Record<string, string> | undefined),
        ...(affiliate.affiliateIds ?? {}),
      },
      internalNotes: affiliate.internalNotes ?? (existingMeta.internalNotes as string | null) ?? null,
    };
  }
