...
export default function MarketingNav() {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 border-b border-neutral-200/60 bg-[#fbf7f4]/80 supports-[backdrop-filter]:backdrop-blur supports-[backdrop-filter]:bg-[#fbf7f4]/60 py-3 sm:py-4 transition-shadow duration-200 ${
        scrolled ? "shadow-[0_20px_50px_rgba(62,47,53,0.15)]" : ""
      }`}
    >
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-10 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="space-y-1 text-ellipsis whitespace-nowrap">
          ...
          <p className="mt-1 text-[11px] sm:text-[12px] tracking-[0.22em] uppercase text-neutral-600">
            Baby Co.
          </p>
        </Link>
        ...
        <ul className="hidden lg:flex items-center gap-8">
          ...
          <li className="relative">
            <button
              type="button"
              className={`flex items-center gap-2 rounded-full px-3 py-2 text-[12px] tracking-[0.28em] uppercase transition-colors duration-200 ${
                experienceActive
                  ? `text-neutral-900 ${activeUnderline}`
        ...
        <div className="hidden lg:flex">
          <div className="flex items-center gap-4">
            <Link href={PUBLIC_LOGIN_ROUTE} className={subtleNavLink}>
              Login
            </Link>
