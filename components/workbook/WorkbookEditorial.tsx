...
const WorkbookEditorial = ({ module }: WorkbookEditorialProps) => {
  ...
  const checklistSeed = moduleContent.apply ?? [];
  const journalPrompt = toParagraphString(moduleContent.journalPrompt);
  const exploreCopy = toParagraphString(moduleContent.explore);
  const lectureCopy = moduleContent.lectureSlides?.length
    ? moduleContent.lectureSlides.join('\n\n')
    : toParagraphString(moduleContent.lecture);
  ...
  const handleTileSave = async (tile: MoodboardTileData) => {
    try {
      await pinterest.savePin({
        ...
        note: tile.caption || module.subtitle || 'Moodboard inspiration',
        title: module.title,
        link: tile.link,
      });
    } catch {
