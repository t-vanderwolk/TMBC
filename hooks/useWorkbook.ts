...
export const getDefaultMoodboardPack = (module: MoodboardModuleSeed): Omit<MoodboardTile, 'id'>[] => [
  ...
  { imageUrl: '/academy/textures/boucle.jpg', caption: 'Bouclé softness', size: 'medium' },
  {
    ...
    caption: module.subtitle ?? 'Moodboard focal',
    size: 'large',
  },
];
...
export const useWorkbook = ({ moduleId, checklistSeed = [], moodboardSeed = [] }: UseWorkbookOptions) => {
  ...
  const persistEntry = useCallback(
    async (type: WorkbookEntrySectionType, content: unknown) => {
      ...
      try {
        ...
        const res = existing
          ? await api.patch(`/workbook/update/${existing.id}`, { content })
          : await api.post('/workbook/create', { moduleId, type, content });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await api.get(`/workbook/list?moduleId=${encodeURIComponent(moduleId)}`);
        if (!mounted) return;
