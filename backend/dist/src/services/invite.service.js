...
const enforceMentorLimit = async (creatorId, role) => {
    if (role !== client_1.Role.MENTOR)
        return;
    const activeCount = await client_2.prisma.invite.count({
...
const generateInvite = async ({ creatorId, email, role, expiresAt, maxUses = 1, }) => {
    const creator = await client_2.prisma.user.findUnique({ where: { id: creatorId } });
    if (!creator) {
    ...
    return client_2.prisma.invite.create({
        data: {
...
const validateInvite = async (code) => {
    const invite = await client_2.prisma.invite.findUnique({ where: { code } });
    if (!invite) {
...
const consumeInvite = async ({ code, email, password, name, }) => {
    ...
    await client_2.prisma.invite.update({
        where: { id: invite.id },
        data: {
            used: true,
            usedAt: new Date(),
...
const getAllInvites = () => {
    return client_2.prisma.invite.findMany({
