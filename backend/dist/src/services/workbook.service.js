...
const listWorkbookEntries = async (params) => {
    const entries = await client_1.prisma.workbookEntry.findMany({
...
const updateWorkbookEntry = async (payload) => {
    const existing = await client_1.prisma.workbookEntry.findUnique({
        where: { id: payload.id },
    });
    if (!existing || existing.userId !== payload.userId) {
        throw new Error('Workbook entry not found');
    }
    const entry = await client_1.prisma.workbookEntry.update({
        where: { id: payload.id },
        data: {
            content: payload.content,
        },
...
const deleteWorkbookEntry = async (payload) => {
    ...
    await client_1.prisma.workbookEntry.delete({ where: { id: payload.id } });
    return true;
};
