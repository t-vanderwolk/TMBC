...
const recordConflicts = async (conflicts) => {
    ...
    await Promise.all(conflicts.map(async (conflict) => {
        ...
        if (normalizedLocal === normalizedRemote) {
            await client_2.prisma.registryConflict.updateMany({
                where: {
        ...
        const existing = await client_2.prisma.registryConflict.findFirst({
            where: {
                userId: conflict.userId,
                itemId: conflict.itemId,
                field: conflict.field,
        ...
        if (existing) {
            await client_2.prisma.registryConflict.update({
        ...
        await client_2.prisma.registryConflict.create({
            data: {
                userId: conflict.userId,
                itemId: conflict.itemId,
                field: conflict.field,
...
const listActiveConflicts = async (userId) => {
    return client_2.prisma.registryConflict.findMany({
...
const getConflictById = async (conflictId, userId) => {
    return client_2.prisma.registryConflict.findFirst({
...
const applyRemoteValue = async (conflictId, userId) => {
    const conflict = await client_2.prisma.registryConflict.findFirst({
