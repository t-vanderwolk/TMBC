...
const fetchMenteesViaInvites = async (mentorId) => {
    const invites = await client_2.prisma.invite.findMany({
...
const fetchSharedJournalEntries = async (mentorId, memberId) => {
    const shares = await client_2.prisma.journalShare.findMany({
...
const getMentorOverview = async (mentorId) => {
    try {
        const [tasks, feedback, mentees] = await Promise.all([
            client_2.prisma.mentorTask.findMany({
                where: { mentorId, completed: false },
                orderBy: { createdAt: 'asc' },
            }),
            client_2.prisma.mentorFeedback.findMany({
                where: { mentorId },
                include: {
...
const getMenteeList = async (mentorId) => {
    try {
        ...
        const fallbackMembers = await client_2.prisma.user.findMany({
...
const getMemberProfileForMentor = async (mentorId, memberId) => {
    try {
        const member = await client_2.prisma.user.findUnique({
            where: { id: memberId },
            select: { id: true, name: true, email: true, createdAt: true },
        });
        if (!member) {
...
const addMentorFeedback = async ({ mentorId, memberId, moduleId, registryItemId, message }) => {
    try {
        const entry = await client_2.prisma.mentorFeedback.create({
...
const getFeedbackForMember = async (memberId, registryItemId, moduleId) => {
    try {
        const feedback = await client_2.prisma.mentorFeedback.findMany({
            where: {
                memberId,
...
const createTask = async ({ mentorId, memberId, type, referenceId, title, description }) => {
    try {
        const task = await client_2.prisma.mentorTask.create({
...
const completeTask = async (taskId) => {
    try {
        const task = await client_2.prisma.mentorTask.update({
            where: { id: taskId },
            data: { completed: true },
        });
        return { ok: true, data: task };
...
const getTasksForMentor = async (mentorId) => {
    try {
        const tasks = await client_2.prisma.mentorTask.findMany({
            where: { mentorId },
            orderBy: { createdAt: 'desc' },
        });
...
const shareJournalEntry = async ({ journalId, memberId, mentorId, allowed = true }) => {
    try {
        const existing = await client_2.prisma.journalShare.findFirst({
            where: { journalId, memberId, mentorId },
        });
        if (existing) {
            const share = await client_2.prisma.journalShare.update({
                where: { id: existing.id },
                data: { allowed },
            });
            return { ok: true, data: share };
        }
        const share = await client_2.prisma.journalShare.create({
