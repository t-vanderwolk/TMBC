...
const listRegistryItems = async (userId, mentorRef) => {
    const items = await client_2.prisma.registryItem.findMany({
        where: { userId },
...
const syncMyRegistryAdd = async (userId, item) => {
    ...
    try {
        ...
        if (giftId) {
            await client_2.prisma.registryItem.update({
                where: { id: item.id },
...
const addRegistryItem = async ({ userId, productId, quantity = 1, notes, status = client_1.RegistryStatus.NEEDED, mentorRef, }) => {
    const product = await client_2.prisma.product.findUnique({ where: { id: productId } });
    if (!product) {
...
const addCustomItem = async ({ userId, title, url, merchant, price, image, category, moduleCode, }) => {
    ...
    const created = await client_2.prisma.registryItem.create({
        data: {
            userId,
            isCustom: true,
            title,
...
const updateRegistryItem = async ({ itemId, userId, quantity, notes, status, purchaseSource, mentorRef, }) => {
    const item = await client_2.prisma.registryItem.findFirst({
    ...
    const updated = await client_2.prisma.registryItem.update({
        where: { id: itemId },
        data: {
            quantity,
            notes,
...
const removeRegistryItem = async (itemId, userId) => {
    ...
    await client_2.prisma.registryItem.delete({ where: { id: itemId } });
...
const createMentorNote = async ({ memberId, mentorId, productId, note }) => {
    const mentorNote = await client_2.prisma.mentorNote.create({
        data: {
            memberId,
            mentorId,
            productId,
