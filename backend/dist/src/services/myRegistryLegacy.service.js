...
const syncDown = async (userId) => {
    try {
        ...
        const localItems = await client_2.prisma.registryItem.findMany({
            where: { userId },
        ...
        for (const remote of remoteItems) {
            ...
            if (existing) {
                ...
                await client_2.prisma.registryItem.create({
                    data: {
                        userId,
        ...
        const orphaned = localItems
            ...
            .map((item) => item.id);
        if (orphaned.length) {
            await client_2.prisma.registryItem.updateMany({
        ...
        const refreshedItems = await client_2.prisma.registryItem.findMany({
            where: { userId },
            include: { product: true },
...
const isConnected = async (userId) => {
    ...
    const existing = await client_2.prisma.registryItem.findFirst({
