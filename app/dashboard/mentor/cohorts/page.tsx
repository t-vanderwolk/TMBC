...
export default function MentorCohortsPage() {
  ...
  const handleCreateCohort = async (event: FormEvent) => {
    ...
    try {
      const response = await api.post('/events/cohorts', { name: cohortName });
  ...
  const handleAddMember = async (event: FormEvent) => {
    ...
    try {
      const response = await api.post(`/events/cohorts/${selectedCohort}/add-member`, { memberId });
      setCohorts((prev) =>
        prev.map((cohort) =>
