...
import { buildCuratedRegistry, CuratedCategory } from "@/lib/registry/recommendations";
import { PUBLIC_LOGIN_ROUTE } from "@/lib/auth/routeForRole";
import { type AcademyModule, type ModuleSection } from "../modules";
...
const toSections = (module: AcademyModule | null): ModuleSection[] => {
  if (!module) return [];
...
const mentorNotesList = (module: AcademyModule | null) => {
  if (!module) return [];
...
export default function ModuleDetailPage({ params }: ModulePageProps) {
  ...
  const [suggestions, setSuggestions] = useState<CuratedCategory[]>([]);
  ...
  useEffect(() => {
    void buildCuratedRegistry(["nesting", "care"]).then((registry) => {
      setSuggestions(registry.categories.slice(0, 3));
    });
  }, []);
